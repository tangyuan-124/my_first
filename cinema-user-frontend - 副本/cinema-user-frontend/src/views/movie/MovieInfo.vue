    ...
    <div class="header">
      <div class="header-inner clearfix">
        ...
        <div class="movie-info-right">
          ...
          <!-- 判断是否已经过期 -->
          <div class="movie-info-btn" v-if="canShowDiscountButton">
...
export default {
  ...
  computed: {
    // 计算是否可以显示特惠购票按钮
    canShowDiscountButton() {
      if (!this.movieInfo.releaseDate) return true; // 如果没有上映日期，默认显示特惠购票按钮

      // 获取当前日期
      const currentDate = new Date();

      // 将 movieInfo.releaseDate 转换为 Date 类型，假设 releaseDate 是 'YYYY-MM-DD' 格式
      const releaseDate = new Date(this.movieInfo.releaseDate);

      // 获取上映时间后的两个月日期
      const twoMonthsLater = new Date(releaseDate);
      twoMonthsLater.setMonth(releaseDate.getMonth() + 6);

      // 如果当前时间晚于上映时间两个月后的日期，则不显示特惠购票按钮
  ...
  methods: {
    ...
    toChooseSession(){
      let cinemaId = 1
      //let movieId = this.movieInfo.movieId; // 获取当前电影的 ID
      //let cinemaId = this.movieInfo.cinemaId;
